...
class BuildingDesignCore:
    ...
    def seed_pilot(self, graph: ProjectGraph) -> ProjectGraph:
        site = graph.add_node("site", "Pilot Site", jurisdiction="VE", units="SI")
        level = graph.add_node("level", "Level 1", elevation=0.0)
        ...
        wall = graph.add_node("wall", "Wall W1", thickness_m=0.2)
        door = graph.add_node("door", "Door D1", width_m=0.9)
        window = graph.add_node("window", "Window WN1", width_m=1.2)
