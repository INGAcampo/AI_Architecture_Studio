...
class ReinforcementEngine:
    ...
    def build(self, structural_result, standards_evidence: dict, project_id="PILOT-BUILDING-001") -> ReinforcementModel:
        ...
        result=ReinforcementModel(project_id)
        for eid, check in structural_result.design_checks.items():
            ...
            common={"element_id":eid,"element_type":kind,"status":status,"diameter_mm":diameter,"cover_mm":40,"development_length_mm":40*diameter,"lap_length_mm":50*diameter,"hook":"90deg","rule":"VE-PILOT-001.0","calculated_as_mm2":as_req}
        ...
        for bar in result.bar_sets:
            kg=bar["quantity"]*bar["length_m"]*(bar["diameter_mm"]**2/162); result.schedules.append({"bar_mark":bar["bar_mark"],"element_id":bar["element_id"],"diameter_mm":bar["diameter_mm"],"quantity":bar["quantity"],"length_m":bar["length_m"],"weight_kg":round(kg,3),"status":bar["status"],"sha256":bar["sha256"]})
        return result
