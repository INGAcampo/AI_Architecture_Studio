...
def test_chain_skips_certified_gates_and_starts_next_pending(monkeypatch, tmp_path):
    ...
    assert calls==['ARCHITECTURAL_PRODUCTION_CORE_READY','STRUCTURAL_PRODUCTION_CORE_READY','ANALYSIS_PRODUCTION_CORE_READY','STANDARDS_PRODUCTION_CORE_READY','DESIGN_PRODUCTION_CORE_READY','DRAWINGS_PRODUCTION_CORE_READY']
    assert state['head']=='abc'
    assert state['next_target']=='QUANTITIES_PRODUCTION_CORE_READY'
...
def test_drawings_target_continues_from_design(tmp_path, monkeypatch):
    def fake_certify(output):
        return {
            'verdict':'DRAWINGS_PRODUCTION_CORE_READY',
            'checks':{'bar_marks_materialized':True},
            'projects':[{'evidence_file':'A.json'},{'evidence_file':'B.json'}],
        }
    import aias_project_production.certification
    monkeypatch.setattr(
        aias_project_production.certification,
        'certify_drawings_production_core',
        fake_certify,
    )
    supervisor=AIASAutonomousSupervisor(tmp_path); supervisor._head=lambda:'abc'
    state=supervisor.run(target='DRAWINGS_PRODUCTION_CORE_READY')
    assert state['status']=='TARGET_REACHED'
    assert state['gates_pass'][-2:]==[
        'DESIGN_PRODUCTION_CORE_READY',
        'DRAWINGS_PRODUCTION_CORE_READY',
    ]
