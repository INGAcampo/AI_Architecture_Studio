...
ROOT=Path(__file__).resolve().parents[1]; TARGET=ROOT/"AIAS_AEC_ORCHESTRATOR001_INSTALLER"
def main():
    if TARGET.exists(): shutil.rmtree(TARGET)
    payload=TARGET/"payload"
    for rel in ("src/aias_aec_orchestrator","engineering/aias/aec_orchestrator","docs/AEC_ORCHESTRATOR_001.md","tests/test_aec_orchestrator001.py"):
        s,d=ROOT/rel,payload/rel; d.parent.mkdir(parents=True,exist_ok=True); shutil.copytree(s,d) if s.is_dir() else shutil.copy2(s,d)
    (TARGET/"manifest.json").write_text(json.dumps({"pack_id":"AEC-ORCHESTRATOR-001","version":"1.0.0","status":"CONTRACT_ACTIVATED","runtime_integrations":"PENDING"},indent=2)+"\n",encoding="utf-8")
