...
ROOT=Path(__file__).resolve().parents[1]; TARGET=ROOT/"AIAS_NEXT008_RELEASE_PROMOTION_INSTALLER"
def main():
    if TARGET.exists(): shutil.rmtree(TARGET)
    payload=TARGET/"payload"
    for rel in ("src/aias_release_promotion","engineering/aias/release_promotion","docs/AIAS_NEXT_008_RELEASE_PROMOTION.md","tests/test_aias_next008_promotion.py"):
        s,d=ROOT/rel,payload/rel; d.parent.mkdir(parents=True,exist_ok=True); shutil.copytree(s,d) if s.is_dir() else shutil.copy2(s,d)
    (TARGET/"manifest.json").write_text(json.dumps({"pack_id":"AIAS-NEXT-008","version":"1.0.0","status":"CANDIDATE_NOT_PROMOTED","promotion_gates":"open"},indent=2)+"\n",encoding="utf-8")
