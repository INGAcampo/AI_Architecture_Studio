...
ROOT=Path(__file__).resolve().parents[1]; TARGET=ROOT/"AIAS_NEXT002_CROSS_PACKAGE_RELEASE_INSTALLER"
def main():
    if TARGET.exists(): shutil.rmtree(TARGET)
    payload=TARGET/"payload"
    for rel in ("engineering/aias/release_gate","tests/test_aias_next002_release_gate.py","docs/AIAS_NEXT_STRATEGY.md"):
        s,d=ROOT/rel,payload/rel; d.parent.mkdir(parents=True,exist_ok=True); shutil.copytree(s,d) if s.is_dir() else shutil.copy2(s,d)
    (TARGET/"manifest.json").write_text(json.dumps({"pack_id":"AIAS-NEXT-002","version":"1.0.0","status":"CONTRACTS_VALIDATED_EXTERNAL_GATES_PENDING","tested_contracts":8},indent=2)+"\n",encoding="utf-8")
