...
ROOT=Path(__file__).resolve().parents[1]; TARGET=ROOT/"AIAS_NEXT006_EVIDENCE_AUDIT_INSTALLER"
def main():
    if TARGET.exists(): shutil.rmtree(TARGET)
    payload=TARGET/"payload"
    for rel in ("src/aias_evidence_audit","engineering/aias/evidence_audit","docs/AIAS_NEXT_006_EVIDENCE_AUDIT.md","tests/test_aias_next006_audit.py"):
        s,d=ROOT/rel,payload/rel; d.parent.mkdir(parents=True,exist_ok=True); shutil.copytree(s,d) if s.is_dir() else shutil.copy2(s,d)
    (TARGET/"manifest.json").write_text(json.dumps({"pack_id":"AIAS-NEXT-006","version":"1.0.0","status":"HASH_CHAIN_VALIDATED","registry_sync":"PENDING"},indent=2)+"\n",encoding="utf-8")
